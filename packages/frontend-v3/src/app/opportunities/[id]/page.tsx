 ...
 * fallback (nginx serves this route's `__placeholder__` detail shell for any
 * `/opportunities/<id>`), which then client-renders the real id resolved from
 * `window.location` (see `client.tsx` — `useParams()` yields the build-time
 * placeholder, so the id is read from the path). The legacy `/opportunities?id=<id>`
 * query form still resolves to the same detail view.
 */
