...
async function seed() {
  ...
  try {
    ...
    // --- Risk Register (must come after opportunities for FK integrity) ---
    const riskSeeds = [
      { id: 'risk-001', opp_id: 'opp-001', opp_title: 'IEWS SETA Support', category: 'competitive', if_s: 'If incumbent (Leidos) submits aggressive pricing', then_s: 'Then our cost proposal may be non-competitive despite technical superiority', likelihood: 'high', impact: 'high', score: 20, status: 'open', mitigation: 'Develop competitive pricing model with past performance justification; identify cost efficiencies from existing tools', owner: 'Capture Manager', triggers: '{"Leidos hires additional BD staff","Leidos wins related SETA contract","Industry day attendance suggests strong incumbent presence"}', due: '2026-06-15' },
      { id: 'risk-002', opp_id: 'opp-003', opp_title: 'Cyber Training Range', category: 'technical', if_s: 'If the government requires FedRAMP High authorization', then_s: 'Then timeline extends 6-9 months and compliance costs increase by $500K+', likelihood: 'medium', impact: 'high', score: 15, status: 'mitigating', mitigation: 'Pre-position FedRAMP documentation; partner with authorized cloud provider; begin ATO package early', owner: 'Technical Lead', triggers: '{"RFP mentions FedRAMP High","Draft SOW references cloud security requirements","Agency issues FedRAMP memo"}', due: '2026-07-01' },
      { id: 'risk-003', opp_id: 'opp-002', opp_title: 'Next-Gen Network Monitoring', category: 'schedule', if_s: 'If key personnel are unavailable during proposal period', then_s: 'Then proposal quality degrades and win probability drops by 15-20%', likelihood: 'medium', impact: 'medium', score: 12, status: 'open', mitigation: 'Identify backup SMEs; pre-write technical sections; maintain proposal-ready resumes', owner: 'HR Director', triggers: '{"Team member gives notice","Conflicting proposal deadlines","Key personnel assigned to active project surge"}', due: '2026-05-30' },
      { id: 'risk-004', opp_id: null, opp_title: null, category: 'regulatory', if_s: 'If CMMC 2.0 Level 2 certification is delayed beyond Q3 2026', then_s: 'Then company is ineligible for 40% of target DoD opportunities requiring CMMC compliance', likelihood: 'low', impact: 'high', score: 10, status: 'mitigating', mitigation: 'Accelerate CMMC assessment prep; engage C3PAO early; conduct gap analysis monthly', owner: 'CISO', triggers: '{"C3PAO scheduling delays","NIST SP 800-171 assessment gaps found","CUI handling deficiencies identified"}', due: '2026-09-01' },
      { id: 'risk-005', opp_id: 'opp-005', opp_title: 'Data Analytics Platform', category: 'teaming', if_s: 'If teaming partner (small business) loses its set-aside eligibility', then_s: 'Then joint venture structure becomes invalid and re-proposal is required', likelihood: 'low', impact: 'medium', score: 6, status: 'accepted', mitigation: 'Monitor partner SBA status quarterly; maintain backup teaming arrangements; structure agreements with contingency clauses', owner: 'Contracts Manager', triggers: '{"Partner revenue exceeds size standard","SBA re-certification fails","Partner acquired by large business"}', due: '2026-08-15' },
      { id: 'risk-006', opp_id: 'opp-004', opp_title: 'Cloud Migration Services', category: 'cost', if_s: 'If cloud infrastructure costs increase more than 15% year-over-year', then_s: 'Then fixed-price contract margins erode below breakeven within 18 months', likelihood: 'medium', impact: 'medium', score: 9, status: 'open', mitigation: 'Include escalation clauses in proposal; negotiate reserved capacity pricing; build 10% cost contingency', owner: 'Finance Director', triggers: '{"AWS/Azure announce price increases","Spot instance pricing volatility exceeds 20%","Government adds new compliance requirements increasing compute needs"}', due: '2026-06-30' },
      { id: 'risk-007', opp_id: null, opp_title: null, category: 'past_performance', if_s: 'If CPARS rating drops below Satisfactory on current DISA contract', then_s: 'Then past performance evaluation scores decrease across all new proposals', likelihood: 'low', impact: 'high', score: 10, status: 'mitigating', mitigation: 'Conduct monthly performance reviews with COR; address all issues within 48 hours; maintain CPARS-ready documentation', owner: 'Program Manager', triggers: '{"COR raises performance concern","Deliverable rejected","SLA breach occurs"}', due: '2026-12-31' },
    ];
    for (const r of riskSeeds) {
      await client.query(
        ...
        [r.id, r.opp_id, r.opp_title, r.category, r.if_s, r.then_s, r.likelihood, r.impact, r.score, r.status, r.mitigation, r.owner, r.triggers, r.due]
      );
    }
    process.stdout.write(`[seed] ${riskSeeds.length} risk register entries\n`);
