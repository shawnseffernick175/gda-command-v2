...
import govtribeRouter from "./routes/govtribe";
...
import opportunityAnalysisRouter from "./routes/opportunities/analysis";
...
// Paths with their own auth handling — skip global JWT middleware.
...
const SELF_AUTH_PREFIXES = ["/auth", "/ingest", "/sentinel", "/launchpad", "/v2", "/partner-intel", "/captures", "/action-items", "/compliance-items"];
...
app.use("/api/opportunities", opportunityAnalysisRouter);
...
app.use("/api/govtribe", govtribeRouter);
