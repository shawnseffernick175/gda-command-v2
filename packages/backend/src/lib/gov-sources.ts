...
export async function pollGovTribeMCP(): Promise<GovTribePollResponse> {
  ...
  for (const search of GOVTRIBE_POLL_SEARCHES) {
    try {
      ...
      trackCredits(search.tool, records.length);
      ...
      for (const r of records) {
        ...
        // Map to ingest-compatible format (same shape POST /api/ingest/govtribe expects)
      ...
      log.info("govtribe_poll_search", { search: search.name, count: records.length, total: parsed.total ?? 0 });
    } catch (e) {
