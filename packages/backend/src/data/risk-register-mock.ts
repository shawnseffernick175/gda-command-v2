...
export const MOCK_RISK_REGISTER: RiskRegisterEntry[] = [
  {
    ...
    opportunity_title: "PEO IEW&S SETA Support Services IDIQ",
    category: "teaming",
    if_statement: "CyberEdge teaming agreement does not close by May 15",
    then_statement: "Proposal will lack state DISA coordination capability, reducing technical score by 10-15 points",
    likelihood: "medium",
    impact: "high",
    risk_score: riskScore("medium", "high"),
    status: "mitigating",
    mitigation_plan: "Parallel negotiations with CyberEdge Solutions as backup sub. Weekly status calls with CyberEdge legal.",
    mitigation_owner: "Shawn Seffernick",
    trigger_indicators: [
      "No signed LOI by May 10",
      "Legal review takes > 5 business days",
      "CyberEdge requests exclusivity terms",
    ],
    contingency_plan: "Execute CyberEdge Solutions teaming agreement within 48 hours. CyberEdge has pre-negotiated terms ready.",
    due_date: "2026-05-15",
  ...
  },
  {
    ...
    opportunity_title: "PEO IEW&S SETA Support Services IDIQ",
    category: "regulatory",
    if_statement: "DISA finalizes new STIG validation MCL standards before RFP closes",
    ...
    trigger_indicators: [
      "DISA publishes final rule in Federal Register",
      "OMB completes review of proposed rule",
      "Congressional hearing scheduled on STIG validation standards",
    ],
  ...
  },
  {
    ...
    opportunity_title: "Air Force DEVCOM C5ISR Cyber IA Services — Phase 3",
    category: "competitive",
    if_statement: "Hensel Phelps leverages existing construction contract to expand into defense IT scope",
    then_statement: "Government may sole-source defense IT work to Hensel Phelps, eliminating competitive opportunity",
    likelihood: "medium",
    impact: "high",
    risk_score: riskScore("medium", "high"),
    status: "open",
    mitigation_plan: "Demonstrate through industry day briefing that defense IT scope requires specialized NIST 800-53 expertise not available through construction contractor.",
    mitigation_owner: "BD Coordinator",
    trigger_indicators: [
      "Hensel Phelps hires defense IT staff",
      "Base issues RFI combining construction and defense IT",
      "Cyber IA scope referenced in contract modification",
    ],
  ...
  },
  {
    ...
    then_statement: "Envision cannot bid as prime; must restructure as sub to an 8(a) firm, reducing control and margin",
    likelihood: "medium",
    impact: "high",
    risk_score: riskScore("medium", "high"),
    status: "mitigating",
    mitigation_plan: "Pre-identified 8(a) teaming partner (CyberEdge Solutions). Draft mentor-protégé agreement prepared.",
    mitigation_owner: "Shawn Seffernick",
    trigger_indicators: [
    ...
    contingency_plan: "Execute mentor-protégé agreement with CyberEdge. Envision performs 40% as sub.",
    due_date: "2026-05-20",
  ...
  },
  {
    ...
    opportunity_title: "Hanscom AFB Launch Complex Modernization",
    category: "past_performance",
    if_statement: "Evaluation team weights NASA past performance above technical approach",
    then_statement: "Envision scores poorly on past performance (no NASA contracts), dropping below competitive range",
    likelihood: "high",
    impact: "high",
    risk_score: riskScore("high", "high"),
    status: "mitigating",
    mitigation_plan: "JV with Jacobs who has 10+ years Hanscom AFB experience. Structure JV so Jacobs past performance qualifies the team.",
    mitigation_owner: "BD Coordinator",
    trigger_indicators: [
  ...
  },
  {
    ...
    opportunity_title: "DISA SETA Technical Support — Region 4",
    category: "competitive",
    if_statement: "SAIC submits significantly lower price based on incumbent efficiencies",
    then_statement: "Government selects SAIC despite lower technical score, as LPTA evaluation favors price",
    likelihood: "high",
    impact: "high",
    risk_score: riskScore("high", "high"),
    status: "open",
    mitigation_plan: "Identify cost efficiencies through innovation (AI-powered security assessment, remote sensing). Target 15% cost reduction vs. traditional approach.",
    mitigation_owner: "Pricing Manager",
    trigger_indicators: [
      "RFP confirms LPTA evaluation methodology",
      "SAIC posts job listings for Region 4 staff",
  ...
  },
  {
    ...
    opportunity_title: "DOE CECOM Decommissioning & Demolition Support",
    category: "schedule",
    if_statement: "DOE delays RFP release beyond July 2026",
    then_statement: "Proposal team availability conflicts with PEO IEW&S SETA proposal preparation, splitting resources",
    likelihood: "medium",
    impact: "medium",
  ...
  },
  {
    ...
    trigger_indicators: [
      ...
      "Labor market tightens for defense IT engineers",
    ],
  ...
  },
  {
    ...
    opportunity_title: "Navy STIG validation Investigation & Remedial Action — NAS Jacksonville",
    category: "regulatory",
    if_statement: "DoD issues interim STIG validation cleanup guidance that differs from DISA standards",
    then_statement: "Remedial approach in proposal may not align with updated DoD requirements, requiring technical rewrite",
    likelihood: "medium",
    impact: "medium",
    risk_score: riskScore("medium", "medium"),
    status: "open",
    mitigation_plan: "Track DoD STIG validation Task Force announcements. Design remedial approach flexible enough to accommodate both DISA and DoD standards.",
    mitigation_owner: "Technical Lead",
    trigger_indicators: [
      "DoD STIG validation Task Force publishes new guidance",
      "Navy issues STIG validation-specific systems engineering directive",
      "Congress includes STIG validation cleanup funding in NDAA",
    ],
  ...
  },
  {
    ...
    opportunity_title: "PEO IEW&S SETA Support Services IDIQ",
    category: "technical",
    if_statement: "USACE requires specific STIG validation destruction technology not yet commercially available",
    then_statement: "Cannot meet mandatory technical requirement, rendering proposal non-compliant",
    likelihood: "low",
    impact: "high",
    risk_score: riskScore("low", "high"),
    status: "mitigating",
    mitigation_plan: "Partner with Battelle (STIG validation Annihilator developer) and include technology maturation plan in proposal. Pre-qualify with USACE EM.",
    mitigation_owner: "Technical Lead",
    trigger_indicators: [
      "RFP mandates specific STIG validation destruction technology",
      "DISA approves STIG validation destruction technology standard",
  ...
  },
  {
    ...
    contingency_plan: "Shift resources to state/commercial defense IT work during shutdown. Resume federal captures upon reopening.",
    due_date: null,
