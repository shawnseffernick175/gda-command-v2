...
export async function upsertOpportunityWithSources(
  ...
  try {
    ...
    // Create a per-notice source row in the sources table (R1 backbone)
    ...
    // Write per-field source citations
    for (const citation of citations) {
      const table = FIELD_TO_TABLE[citation.field];
      if (!table) continue;

      // Insert citation, skip if this source already exists for this opp
      await client.query(
