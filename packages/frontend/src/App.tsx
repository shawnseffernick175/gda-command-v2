...
export default function App() {
  ...
  return (
    ...
    <ToastProvider>
    <StagingBanner />
    <div className="min-h-screen bg-bg text-ink font-sans flex">
      ...
      >
        ...
        >
          <Link
            to="/"
            className="font-bold no-underline text-ink whitespace-nowrap overflow-hidden"
            style={{ fontSize: sidebarOpen ? 16 : 0, letterSpacing: "0.5px", transition: "font-size 0.2s" }}
          >
        ...
        <nav className="flex-1 overflow-auto py-2">
          {NAV_GROUPS.map((group) => (
            <div key={group.label} className="mb-2">
              {sidebarOpen && (
                <div className="text-[10px] font-semibold text-muted uppercase tracking-widest px-4 pt-2 pb-1 opacity-60">
                  {group.label}
                </div>
              )}
              {!sidebarOpen && (
              ...
              {group.items
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2.5 text-[13px] no-underline whitespace-nowrap overflow-hidden transition-colors duration-150 ${
                      active ? "font-semibold text-accent border-l-2 border-accent" : "font-normal text-muted"
                    }`}
                    style={{
        ...
        >
          {sidebarOpen ? (
            <>
              <span className="text-caption text-muted flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
        ...
        {isMobile && (
          <div className="flex items-center gap-3 px-4 py-2.5 bg-white border-b border-border sticky top-0 z-50">
            <button
              ...
              className="bg-transparent border-none text-ink cursor-pointer text-xl p-1 rounded flex items-center"
              aria-label="Open menu"
            >
              \u2630
            </button>
            <span className="font-bold text-sm">GDA Command</span>
          </div>
        )}

        <FinancialKPIStrip />

        <main className="flex-1" style={{ padding: isMobile ? 16 : 24 }}>
          <Breadcrumb />
...
function AskAnythingFAB() {
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-20 right-4 w-10 h-10 rounded-full bg-accent text-white border-none cursor-pointer text-lg z-[10000] flex items-center justify-center opacity-85"
  ...
  return (
    <div
      className="fixed bottom-20 right-4 w-[380px] max-h-[420px] bg-white border border-border rounded z-[10000] flex flex-col"
    ...
    >
      ...
      <div className="p-3 flex gap-2">
        ...
        <button
          onClick={handleAsk}
          disabled={loading}
          className={`btn-primary btn text-[13px] font-semibold ${loading ? "opacity-50" : ""}`}
        >
          {loading ? "..." : "Ask"}
        </button>
      </div>
