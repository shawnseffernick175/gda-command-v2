...
- **Postgres fallback**: If n8n is unreachable, backend falls back to local DB seeded data. Source badge shows "Live DB" instead of "Live API".
- **Seeded DB data**: 10 opportunities (opp-001 to opp-010), 12 SAM opportunities (sam-001 to sam-012), 15 intel items, 7 risks, 6 morning briefings, 10 FPDS awards
- Envision Innovative Solutions company profile: $382M revenue (Large Business), 41 employees (Small Business by SBA headcount), SDVOSB
