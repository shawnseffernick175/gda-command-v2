...
**Root cause:** Ingest mappers (`mapToDBRecord` in `sam-api.ts`, `govtribe-api.ts`, `govwin-api.ts`, `fpds-api.ts`) return plain `Record<string, unknown>` objects. The INSERT statements in `feed-sync.ts` and `ingest.ts` reference specific columns by positional `$N` parameters. If a mapper produces a field that doesn't exist in the target table, the field is silently discarded — no error, no warning. Conversely, if the schema adds a column the mapper doesn't populate, there's no compile-time or CI-time detection.
