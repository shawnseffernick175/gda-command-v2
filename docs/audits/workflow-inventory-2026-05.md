...
**However:** n8n has a `saveDataSuccessExecution` setting. If set to `none`, successful executions are not saved at all — only errors would appear. This could explain the gap for workflows that fire successfully but silently. The n8n container's environment variables need to be inspected directly (`docker exec n8n env | grep EXECUTIONS`).
...
1. **`saveDataSuccessExecution = none`** — n8n is configured to not save successful execution data. Workflows ARE firing but executions aren't logged. Only errors appear. This explains why `GDA.cron.health-scan-daily` (which succeeds) has 7 records — it might be a recent configuration change, or it might be the only workflow with `saveDataSuccessExecution` overridden at the workflow level.

2. **n8n worker/queue bottleneck** — If n8n is running in queue mode with limited workers, cron triggers may be queuing but never getting processed. The single execution in `new` status (exec ID 100943) is evidence of queued-but-not-started work.

3. **Workflows are genuinely not activating** — A bug, n8n restart that didn't reload schedules, or a timezone mismatch between the server and the cron expressions could cause triggers to never fire.
...
# 5. Force-trigger one cron workflow to see if it logs
# (manual test only — do NOT modify the workflow)
curl -X POST "https://n8n.csr-llc.tech/webhook/[system-watchdog-test-id]"
```

---

## 4. Frozen Workflow Error Analysis — GDA.cron.fast-track-ingest
...
### Also Failing: GDA.api.intel-feed
...
Same timeout pattern. `GDA.api.intel-feed` runs `0 */4 * * *` (every 4 hours) and has failed on both recorded executions. The `Merge Intel` node is also a Code node — likely hitting the same sandbox timeout limit.

### Also Failing: GDA.error.handler

Two `crashed` executions on 2026-05-19. The error handler itself is crashing — which means errors from `intel-feed` and `fast-track-ingest` are firing the error handler, which then also crashes. This is a cascading failure: errors produce more errors.
...
**Proposed approach:** A GitHub Action (`.github/workflows/workflow-inventory.yml`) that runs weekly on Monday at 07:00 UTC (after the drift report at 06:00):
...
5. Alerts fire when:
   - A cron workflow has 0 executions in the last 7 days (expected > 0)
   - A workflow's 7-day success rate drops below 80%
   - A workflow's median duration exceeds 2x its historical baseline

**Implementation cost:** ~100 lines of TypeScript (similar to `check-migration-drift.ts`). Requires `N8N_API_KEY` as a GitHub Actions secret.
