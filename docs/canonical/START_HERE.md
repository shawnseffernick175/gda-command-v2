...
**Bottom line for a new chat:** SSH in (Section 1), read `north_star_roadmap_v3.md`, pick up at the next open ticket (Phase 2 is COMPLETE — next is **Phase 3 Unified UI, starting F-420 detail page**), do the work, open a clean PR, wait for Devin Review to post, fix any legit findings, merge when Devin is resolved and CI is green. End with a recommendation. Don't make Shawn do anything but approve.
