...
- **Phase 2 — Unified API (IN PROGRESS):** ✅ F-410 unified detail endpoint live (`GET /v3/opportunities/unified/:internal_id`) → **F-411 stage filter (NEXT)** → F-412 suggestion queue → F-413 field override + audit.
- **Phase 3 — Unified UI:** F-420 detail page → F-421 tab structure (say-something surfaces) → F-422 review sidebar → F-423 decommission old per-source pages.
...
**Bottom line for a new chat:** SSH in (Section 1), read `north_star_roadmap_v3.md`, pick up at the next open ticket (currently F-411 — stage filter on the list endpoint), do the work, open a clean PR, auto-merge if CI is green. End with a recommendation. Don't make Shawn do anything but approve.
