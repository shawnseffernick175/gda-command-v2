...
- **Phase 2 — Unified API (NEXT):** F-410 `/api/opportunities/:internal_id` detail endpoint → F-411 stage filter → F-412 suggestion queue → F-413 field override + audit.
...
**Bottom line for a new chat:** SSH in (Section 1), read `north_star_roadmap_v3.md`, pick up at the next open ticket (currently F-410), do the work, open a clean PR, auto-merge if CI is green. End with a recommendation. Don't make Shawn do anything but approve.
