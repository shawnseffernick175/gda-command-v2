...
CREATE TABLE pipeline_items (
  ...
  source_id         BIGINT        NOT NULL REFERENCES sources(id),
  created_by        BIGINT        REFERENCES users(id),
  archived_at       TIMESTAMPTZ,               -- when set, hidden from all Pipeline surfaces (reversible)
  created_at        TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
  updated_at        TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);
...
| `archived_at` | Reversible archive flag (nullable). When set, the item is excluded from the board, summary, coverage, and list queries; F-600 forbids deletes, so archiving is how an item leaves the board. `NULL` means active |
