...
import type { IngestResult } from '../../src/ingest/framework/registry.js';
...
describe('Ingest framework — idempotency', () => {
  it('startRun creates a running ingest_runs row', async () => {
    const { startRun } = await import('../../src/ingest/framework/run_logger.js');
  ...
  it('finishRun updates the row with success counters', async () => {
    const { finishRun } = await import('../../src/ingest/framework/run_logger.js');
  ...
  it('registry rejects unknown source keys', async () => {
    const { runIngest } = await import('../../src/ingest/framework/registry.js');
  ...
  it('registry registers and lists sources', async () => {
    vi.resetModules();
    const { registerSource, getRegisteredSources } = await import('../../src/ingest/framework/registry.js');
