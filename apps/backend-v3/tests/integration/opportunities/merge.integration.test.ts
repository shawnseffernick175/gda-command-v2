...
beforeAll(async () => {
  ...
  // ─── 0. Ensure unified_opportunities tables exist ──────────────────────
  // v3_026 migration file includes both UP and DOWN sections; when the test
  // migration runner executes the whole file, the DOWN immediately drops what
  // the UP created. Re-create the tables if they don't exist.
  const { rows: tableCheck } = await pool.query(
    `SELECT to_regclass('public.unified_opportunities') AS tbl`,
  );
  if (!tableCheck[0]?.tbl) {
    await pool.query(`
      DO $$ BEGIN
      ...
      CREATE TABLE IF NOT EXISTS unified_opportunity_links (
      ...
      CREATE TABLE IF NOT EXISTS unified_opportunity_field_overrides (
        id              BIGSERIAL       PRIMARY KEY,
        internal_id     UUID            NOT NULL REFERENCES unified_opportunities(internal_id) ON DELETE CASCADE,
      ...
      CREATE TABLE IF NOT EXISTS unified_opportunity_signals (
        id                  BIGSERIAL       PRIMARY KEY,
        internal_id         UUID            NOT NULL REFERENCES unified_opportunities(internal_id) ON DELETE CASCADE,
