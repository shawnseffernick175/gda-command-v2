...
beforeEach(async () => {
  ...
  await pool.query(`
    ...
    DELETE FROM pipeline_items WHERE opportunity_id IN (SELECT id FROM opportunities WHERE title LIKE 'Test Pipeline%');
    DELETE FROM pipeline_items WHERE capture_owner LIKE 'test-%';
    COMMIT
  `);
