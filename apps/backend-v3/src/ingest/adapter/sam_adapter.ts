/**
 * SAM.gov adapter — normalizes legacy `opportunities` rows where
 * data_source contains 'sam'.
 */
...
export const samAdapter: SourceAdapter = {
  source: 'sam',
  defaultStage: 'solicitation',

  normalize(row: LegacyOpportunityRow): NormalizedOpportunity {
    const nativeId = row.sam_notice_id ?? `sam-legacy-${row.id}`;
